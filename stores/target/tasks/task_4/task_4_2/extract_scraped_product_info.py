# %%
from src.instances import config, task_4_2_logger
...
# %%
"""
(Execute this cell (fetching S3 bucket's JSON files) once per scraping)

Fetch each group's JSON file, parse such list,
and extract its JSON dictionary elements
products_info_json_list = []

bucket_name = config['aws']['s3_bucket_name']
bucket_folder = 'data'

json_objects = s3.list_objects_v2(Bucket = bucket_name, Prefix = bucket_folder)
for obj in json_objects['Contents']:
  response = s3.get_object(Bucket = bucket_name, Key = obj['Key'])
  content = response['Body'].read().decode('utf-8')

  products_info_json_list.extend(json.loads(json.loads(content)))

with open(f'data/products_scraped_info.json', 'w', encoding = 'utf-8') as f:
  json.dump(products_info_json_list, f, ensure_ascii = False, indent = 4)
